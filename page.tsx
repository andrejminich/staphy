...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => base.filter(iso => {
    ...
    return true;
  }), [base, filterPatogen, filterMdr, filterRok, filterOddelenie]);
        ...
        {activeTab === 'heatmapa' && (
          <div>
            ...
            <div style={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(26,95,168,0.08)', padding: '1.5rem', overflowX: 'auto' }}>
                        ...
                        return (
                          <tr key={od}>
                            ...
                            {heatmapData.periods.map(p => {
                              ...
                              const bg = val === 0 ? '#f8fafc' : `rgba(26,95,168,${0.1 + intensity * 0.85})`;
              ...
              {/* Legend */}
              <div style={{ marginTop: 16, display: 'flex', gap: 8, alignItems: 'center', fontSize: 11, color: '#94a3b8' }}>
                <span>Intenzita:</span>
                {[0.1, 0.3, 0.5, 0.7, 0.9].map(i => (
                  <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <span style={{ width: 20, height: 14, borderRadius: 4, background: `rgba(26,95,168,${i})`, display: 'inline-block' }} />
                  </span>
                ))}
                <span>nízka → vysoká · ↑↓ zmena vs predchádzajúce obdobie</span>
              </div>
            </div>
          </div>
        )}
