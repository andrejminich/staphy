...
import { Isolate, computeStats, getTopN, MONTH_NAMES, processRawData, RawRow, EARS_NET, HOSP_TOTAL, getHospCount, ratePer1000, getPandemicPeriod, PANDEMIC_PERIODS } from '@/lib/mdr';
        ...
        {activeTab === 'prehlad' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0,1fr))', gap: 10, marginBottom: '1rem' }}>
              {(() => {
                const lastRokWithHosp = Math.max(...Object.keys(HOSP_TOTAL).map(Number).filter(y => y <= Math.max(...yearlyData.map(d => Number(d.rok)))));
