...
import { Isolate, computeStats, getTopN, MONTH_NAMES, processRawData, RawRow } from '@/lib/mdr';
...
export default function Dashboard() {
  ...
  }, [isolates, filterPatogen, filterMdr, filterMesiac, filterOddelenie, filterRok]);
  ...
  const yearlyData = useMemo(() => {
    const byYear = new Map<number, { mdr: number; nonmdr: number }>();
    for (const iso of isolates) {
      const y = new Date(iso.datumOdberu).getFullYear();
      if (!byYear.has(y)) byYear.set(y, { mdr: 0, nonmdr: 0 });
      const entry = byYear.get(y)!;
      if (iso.isMdr) entry.mdr++; else entry.nonmdr++;
    }
    return Array.from(byYear.entries())
      .sort((a, b) => a[0] - b[0])
      .map(([y, v]) => ({ rok: y.toString(), mdr: v.mdr, nonmdr: v.nonmdr, total: v.mdr + v.nonmdr }));
  }, [isolates]);
  ...
  const availableMonths = useMemo(() => Array.from(new Set(isolates.map(i => i.mesiac))).sort((a, b) => a - b), [isolates]);
  const availableRoky = useMemo(() => Array.from(new Set(isolates.map(i => new Date(i.datumOdberu).getFullYear()))).sort((a, b) => a - b), [isolates]);
  const availableOddelenia = useMemo(() => Array.from(new Set(isolates.map(i => i.oddelenie))).sort(), [isolates]);
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="card space-y-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {([['all', 'Všetky'], ['aureus', 'S. aureus'], ['epidermidis', 'S. epidermidis']] as [FilterPatogen, string][]).map(([val, label]) => (
              <button key={val} onClick={() => setFilterPatogen(val)}
                className={`filter-chip ${filterPatogen === val ? 'filter-chip-active' : 'filter-chip-inactive'}`}>{label}</button>
            ))}
          </div>

          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-xs text-slate-400 w-20">MDR status</span>
            {([['all', 'Všetky'], ['mdr', 'MDR'], ['nonmdr', 'Non-MDR']] as [FilterMdr, string][]).map(([val, label]) => (
              <button key={val} onClick={() => setFilterMdr(val)}
                className={`filter-chip ${filterMdr === val ? 'filter-chip-active' : 'filter-chip-inactive'}`}>{label}</button>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {availableRoky.map(r => (
              <button key={r} onClick={() => setFilterRok(filterRok === r ? null : r)}
        ...
        {/* Monthly Trend */}
        <div className="card">
          <h3 className="font-semibold text-slate-700 mb-4 flex items-center gap-2"><TrendingUp size={16} /> Mesačný trend — MDR vs Non-MDR</h3>
          <ResponsiveContainer width="100%" height={260}>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={(() => {
              ...
              for (const iso of filtered) {
                ...
                if (iso.patogen.includes('aureus')) e.aureus++; else e.epidermidis++;
              }
